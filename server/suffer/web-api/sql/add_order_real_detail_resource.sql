-- 订单补款前查实时数据的接口权限，默认授予管理员角色(role_id = 1)。
...
SELECT 1, NOW(), NOW(), '订单实时数据', 'barryOrderRealDetail', 0,
  'RESOURCE', '/barry/order-real-detail', '', '', '', '', '', 0
WHERE NOT EXISTS (
  SELECT 1 FROM resource_new
  WHERE resource_url = '/barry/order-real-detail' AND active = 1
);
...
FROM resource_new r
WHERE r.resource_url = '/barry/order-real-detail'
  AND r.active = 1
  AND NOT EXISTS (
