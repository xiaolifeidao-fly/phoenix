-- 人工做单明细查询接口权限，默认授予管理员角色(role_id = 1)。
...
SELECT 1, NOW(), NOW(), '人工做单明细查询', 'barryManualOrderDetails', 0,
  'RESOURCE', '/barry/manual-order-details', '', '', '', '', '', 0
WHERE NOT EXISTS (
  SELECT 1 FROM resource_new
  WHERE resource_url = '/barry/manual-order-details' AND active = 1
);
...
FROM resource_new r
WHERE r.resource_url = '/barry/manual-order-details'
  AND r.active = 1
  AND NOT EXISTS (
