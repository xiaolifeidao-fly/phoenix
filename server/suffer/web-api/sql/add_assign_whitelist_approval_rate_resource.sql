-- 分配策略 - 白名单全局审核通过率配置接口，授予管理员角色(role_id = 1)。
...
SELECT 1, NOW(), NOW(), '分配策略-白名单全局审核通过率', 'assignWhitelistApprovalRate', 0,
  'RESOURCE', '/barry/assign-whitelist-approval-rate', '', '', '', '', '', 0
WHERE NOT EXISTS (
  SELECT 1 FROM resource_new
  WHERE resource_url = '/barry/assign-whitelist-approval-rate' AND active = 1
);

INSERT INTO role_resource_new (active, created_time, updated_time, role_id, resource_id)
SELECT 1, NOW(), NOW(), 1, r.id
FROM resource_new r
WHERE r.resource_url = '/barry/assign-whitelist-approval-rate'
  AND r.active = 1
  AND NOT EXISTS (
