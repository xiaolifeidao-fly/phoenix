...
func (h *BarryHandler) getWorkbenchPendingDetectionCount(c *gin.Context) {
	var q barryDTO.WorkbenchDashboardMetricQueryDTO
	if c.ShouldBindQuery(&q) != nil {
		commonRouter.ToError(c, "参数错误")
		return
...
func (h *BarryHandler) getWorkbenchDelayAssignmentCount(c *gin.Context) {
	var q barryDTO.WorkbenchDashboardMetricQueryDTO
	if c.ShouldBindQuery(&q) != nil {
		commonRouter.ToError(c, "参数错误")
		return
	}
	response, err := h.barryService.WorkbenchDashboardStats.DelayAssignmentCount(c.Request.Context(), q)
	commonRouter.ToJson(c, response, err)
}
